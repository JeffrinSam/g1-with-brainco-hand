"""
train_g1_navigate_fixed_goals_example.py

HISTORY SNAPSHOT — preserved on purpose, not meant to be trained "for real."

This is the version of train_g1_navigate.py as it stood right after the
heading-alignment reward fix, using a fixed set of 3 hardcoded goal
positions (GOAL_POSITIONS below), picked randomly per episode from just
those 3. This is the version that was trained and verified in the viewer to
reliably reach all 3 goals (including the one that previously failed against
wall_north).

It was superseded by random goal sampling (see train_g1_navigate.py's
_sample_goal, MIN_GOAL_RADIUS/MAX_GOAL_RADIUS/FURNITURE_EXCLUSION_ZONES) to
test whether the policy had learned real navigation or had just memorized 3
specific routes. Kept here as a before/after reference point for that test.

Everything else in this file (WBC control loop, joint setup, observation/
action spaces, heading-alignment reward) is identical to train_g1_navigate.py
at the time this snapshot was taken -- only goal selection differs from the
current live file. Not committed/pushed; local illustration only.
"""
...
if __name__ == "__main__":
    ...
    print("Starting training (fixed 3-goal version)...")
    model.learn(total_timesteps=100_000)
    model.save("navigation_policy_fixed_goals_example")
    print("Saved to navigation_policy_fixed_goals_example.zip")
